...
/**
 * v2: experienceLevel 別の画像表示に差し替え
 * - Lv0  → /mushroom/lv1.png
 * - Lv1  → /mushroom/lv2.png
 * - Lv2+ → /mushroom/lv3.png
 *
 * 旧 v1 SVG プロシージャル実装はこの関数の下にコメントアウトで保管。
 * 戻す場合は上の新関数を削除し、下のコメント部の各行頭 "// " を一括除去する。
 */
export default function MushroomSVG({
  ...
  const src =
    experienceLevel <= 0 ? "/mushroom/lv1.png" :
    experienceLevel === 1 ? "/mushroom/lv2.png" :
    "/mushroom/lv3.png";

  return (
    <Image
      src={src}
      alt={`Lv${experienceLevel}`}
      width={width}
      height={height}
      className={className}
...
// ============================================================
// 旧実装（v1: SVG プロシージャル描画）
// v2 で画像差し替えに切り替えた。下の行頭 "// " を一括除去すれば復元可能。
// ============================================================
// "use client";
// 
// import { Morphology, GrowthPhase, MushroomSpeciesId } from "@/lib/types";
